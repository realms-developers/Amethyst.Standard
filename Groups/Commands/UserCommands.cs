...
public static class UserCommands
{
    ...
    {
        var list = PagesCollection.PageifyItems(GroupsModule.Users.FindAll(p => p.PersonalPermissions.Any()).Select(p => $"{p.Name} ({p.PersonalPermissions.Count})"), 80);
    ...
    {
        var model = GroupsModule.Users.Find(name);
        if (model == null)
        {
        ...
        var list = PagesCollection.PageifyItems(model.PersonalPermissions, 80);
        var pages = PagesCollection.SplitByPages(list, 10);
    ...
    public static void UserSet(CommandInvokeContext ctx, string name, string groupName, string? time = null)
    {
        ...
        var groupModel = GroupsModule.Groups.Find(groupName);
        if (groupModel == null)
        {
